...
const COMMENTS = getComments().map(comment => {
  return new Comment(comment.name, comment.comment, comment.time);
});
...
const getGuestPage = function(req, res) {
  const tableRows = COMMENTS.map(comment => comment.toHTML()).join('\n');
...
const performCommentSubmission = function(req, res) {
  ...
  COMMENTS.unshift(new Comment(name, comment, new Date()));
  saveComments();
